...
- **مدفوعات حقيقية** (Stripe/PayPal) — الفوترة تبقى **معاينة تجريبية فقط** (الوحيد المتعمد mock):
  - `/dashboard/admin/billing`
  - `/dashboard/company/billing`
  - `/billing`
  - API: `/api/data/billing`
- باقي المنصة تعتمد على **بيانات Supabase الحية** (لوحات الأدمن/الطالب/الشركة من الجداول)
- مقابلات فيديو داخل المنصة (لاحقاً — روابط اجتماع خارجية موجودة)

**مضاف في هذا التجهيز:** Storage لـ CV (`cvs` + `cv_files`)، وحدات/دروس الكورس، كويزات/تقييمات، صور المنشورات ومرفقات الرسائل، تسجيل حضور فعاليات عبر QR، واستعادة كلمة المرور عبر Supabase Auth، وواجهة فوترة/رسوم تجريبية.
